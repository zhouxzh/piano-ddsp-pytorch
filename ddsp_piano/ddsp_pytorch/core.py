...
def crop_and_compensate_delay(audio, 
    ...
    return audio[:, start:] if end == 0 else audio[:, start:-end]
...
def fft_convolve(audio,
    ...
    if frame_size!=audio_size:
        filters = torch.eye(frame_size, device=audio.device, dtype=audio.dtype).unsqueeze(1)
    ...
    if frame_size!=audio_size:
        overlap_add_filter = torch.eye(
            audio_frames_out.size(-1),
            device=audio_frames_out.device,
            dtype=audio_frames_out.dtype,
        ).unsqueeze(1)
