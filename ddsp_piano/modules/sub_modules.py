...
class MonophonicNetwork(nn.Module):
    ...
    def forward(self, conditioning, extended_pitch, context):
        ...
        x = torch.cat([extended_pitch / self.midi_norm,
                       torch.div(conditioning, conditioning.new_tensor([self.midi_norm, 1.])),
                       context],
                       dim=-1) # (b*16, 750, 35)
...
class F0ProcessorCell_RNN(nn.Module):
    ...
    def forward(self, midi_note, previous_state):
        ...
        updated_state = torch.cat([midi_note, decayed_steps], dim=-1)
...
class NoteRelease(nn.Module):
    ...
    def forward(self, conditioning):
        ...
        """
        active_pitch = conditioning[..., 0]
        held_pitch = torch.zeros_like(active_pitch[:, 0])
        released_frames = torch.zeros_like(held_pitch)
        release_limit = self.layer.release_duration.to(active_pitch) * self.layer.frame_rate
        extended = []

        for frame in range(active_pitch.shape[1]):
            current_pitch = active_pitch[:, frame]
            is_active = current_pitch > 0
            released_frames = torch.where(
                is_active,
                torch.zeros_like(released_frames),
                released_frames + 1,
            )
            held_pitch = torch.where(is_active, current_pitch, held_pitch)
            held_pitch = torch.where(
                released_frames <= release_limit,
                held_pitch,
                torch.zeros_like(held_pitch),
            )
            extended.append(held_pitch)

        return torch.stack(extended, dim=1).unsqueeze(-1)
...
class MultiInstrumentReverb(nn.Module):
    ...
    def exponential_decay_mask(self, ir, decay_exponent=4., decay_start=16000):
        ...
        """
        decay_start = min(decay_start, self.reverb_length)
        time = torch.linspace(
            0.0,
            1.0,
            self.reverb_length - decay_start,
            device=ir.device,
            dtype=ir.dtype,
        )
        mask = torch.exp(- decay_exponent * time)
        mask = torch.cat([torch.ones(decay_start, device=ir.device, dtype=ir.dtype), mask], 0)
